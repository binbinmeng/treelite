...
def _obj_cmd(source, toolchain, options):
    obj_ext = _obj_ext()
    return '{} -c -O3 -o {} {} -fPIC -std=c99 {}'.format(
        toolchain, source + obj_ext, source + '.c', ' '.join(options))


def _lib_cmd(objects, target, lib_ext, toolchain, options):
    return '{} -shared -O3 -o {} {} -std=c99 {}'.format(
        toolchain, target + lib_ext, ' '.join(objects), ' '.join(options))
