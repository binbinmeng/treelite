...
def _isascii(string):
    """Tests if a given string is pure ASCII"""
    try:
        return len(string) == len(string.encode('utf-8'))
...
class Model():
    ...
    def load(cls, filename, model_format):
        ...
        if model_format == 'lightgbm':
            ...
            raise ValueError('Unknown model_format: must be one of {lightgbm, xgboost}')
        return Model(handle)
    ...
    **kwargs
    ...
    class Node():
        ...
        def set_leaf_node(self, leaf_value):
            ...
            if not self.empty:
                node_key = self.node_key if self.node_key is not None else '_'
                raise ValueError('Cannot modify a non-empty node. ' +
                                 f'If you meant to change type of node {node_key}, ' +
                                 'delete it first and then add an empty node with the same key.')
