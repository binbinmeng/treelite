...
def _check_call(ret):
    ...
    """
    if ret != 0:
        raise TreeliteError(py_str(_LIB.TreeliteGetLastError()))
...
def _maybe_pandas_data(data, feature_names, feature_types):
    ...
    if not all(dtype.name in PANDAS_DTYPE_MAPPER for dtype in data_dtypes):
        ...
        msg = ('DataFrame.dtypes for data must be in, float, or bool. Did not '
               + 'expect the data types in fields ' + ', '.join(bad_fields))
...
class DMatrix:
    ...
    def __init__(self, data, data_format=None, missing=None,
        ...
        if isinstance(data, (str,)):
            ...
            try:
                ...
                raise TypeError(f'Cannot initialize DMatrix from {type(data).__name__}')
    ...
    def _init_from_csr(self, csr):
        ...
        if len(csr.indices) != len(csr.data):
            raise ValueError('indices and data not of same length: ' +
                             f'{len(csr.indices)} vs {len(csr.data)}')
        if len(csr.indptr) != csr.shape[0] + 1:
            raise ValueError('len(indptr) must be equal to 1 + [number of rows]: ' +
                             f'len(indptr) = {len(csr.indptr)} vs ' +
                             f'1 + [number of rows] = {1 + csr.shape[0]}')
        if csr.indptr[-1] != len(csr.data):
            raise ValueError('last entry of indptr must be equal to len(data)' +
                             f'indptr[-1] = {csr.indptr[-1]} vs ' +
                             'len(data) = {len(csr.data)}')
        self.handle = ctypes.c_void_p()
    ...
    def __repr__(self):
        return (f'<{self.shape[0]}x{self.shape[1]} sparse matrix of type treelite.DMatrix\n' +
