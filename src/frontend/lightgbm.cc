...
inline treelite::Model ParseStream(dmlc::Stream* fi) {
  ...
  if (obj_name_ == "multiclass") {
    ...
    std::strcpy(model.param.pred_transform, "softmax");
    ...
    std::strcpy(model.param.pred_transform, "multiclass_ova");
    ...
    std::strcpy(model.param.pred_transform, "sigmoid");
    model.param.sigmoid_alpha = alpha;
    ...
    std::strcpy(model.param.pred_transform, "logarithm_one_plus_exp");
  } else {
    std::strcpy(model.param.pred_transform, "identity");
  }
